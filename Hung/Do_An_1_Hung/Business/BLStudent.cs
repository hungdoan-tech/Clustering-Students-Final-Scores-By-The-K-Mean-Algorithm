...
namespace Do_An_1.Business
{
    class BLStudent
    {
        public List<dynamic> DetailStudentInfo_cbb_Subjects(string ID, string ProID)
        {
            using (var ctx = new UniversityContext())
            {
                ...
                if (ctx.Professors.SingleOrDefault(s => s.ProfessorID.ToString() == ProID).IsAdmin != true)
                {
                    var detailStudentInfo = (from t in ctx.Students
                                             ...
                                             where r.SubjectID.ToString() == ID && r.ProfessorID.ToString().Contains(ProID)
                                             select new
                                             {
                                                 t.StudentID,
                                                 t.StudentName,
        ...
        public List<dynamic> DetailStudentInfo_cbb_Classes(string ID, string ProID)
        {
            using (var ctx = new UniversityContext())
            {
                if (ctx.Professors.SingleOrDefault(s => s.ProfessorID.ToString() == ProID).IsAdmin != true)
                {
                    var detailStudentInfo = (from t in ctx.Students
                                             ...
                                             join r in ctx.Classes on s.ClassID equals r.ClassID
                                             join q in ctx.Subjects on r.SubjectID equals q.SubjectID
                                             where r.ClassID.ToString() == ID && r.ProfessorID.ToString().Contains(ProID)
                                             select new
                                             {
                                                 t.StudentID,
                                                 t.StudentName,
